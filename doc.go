...
//
// TLS certificates are easy if your server's port 80 is reachable
// from the public internet. Install acmetool from
// https://github.com/hlandau/acme and configure it in redirector
// mode, obtain a certificate ("acmetool want host.example"), and
// configure ListenTLS, CertFile, and KeyFile (see below). There is no
// need to notify or restart hwy3 when the certificate is renewed.
//
// Configuration
//
...
// other channels.
//
// Channels.name.Input: Use the output of another channel as the input
// stream. (If no Input is given, the stream can be injected by piping
// it to "hwy3 -inject name [-chunk 1024]".)
//
// Channels.name.Command: pass the input stream through a shell
// command. The command is restarted automatically if it closes stdout
// or exits.
//
// Channels.name.Calm: minimum number of seconds between successive
// command restarts. Decimals are OK. Must be greater than zero;
// otherwise, defaults to 1.
//
// Channels.name.Chunk: ensure the channel outputs chunks of the given
// size (in bytes). This maintains frame sync for formats like PCM
// that have a fixed frame size.
//
// Channels.name.MP3: ensure the channel outputs whole MP3 frames.
// This maintains frame sync, but it doesn't guarantee a clean stream
// because it doesn't account for the bit reservoir.
//
// Channels.name.Buffers: maximum number of frames/chunks to buffer
// for each listener. When a listener is slow enough to fill all
// buffers, all buffered frames are dropped and the client resumes
// with the current frame.
//
// Channels.name.BufferLow: minimum number of frames/chunks to buffer
// before sending the next frame after a listener underruns its
// buffer.
//
// Channels.name.MP3Dir.Root: directory to read/write mp3 files
// (tNNN.mp3 and current.mp3 where NNN is a unix timestamp
// representing time at EOF)
//
// Channels.name.MP3Dir.BitRate: archived data rate in bits per
// second. On a public channel, this enables the archive-browsing UI,
// and serves archived data at {channelname}/A-B.mp3, where A and B
// are start and end times formatted as decimal UNIX
// timestamps. Requires MP3Dir.BitRate.
//
// Channels.name.MP3Dir.SplitOnSilence: enable writing to mp3dir.
// Start a new output file if no data has been written for the given
// number of nanoseconds.
//
// Channels.name.MP3Dir.SplitOnSize: start a new output file before
// current.mp3 reaches the given number of bytes.
//
// Channels.name.MP3Dir.PurgeOnSize: when starting a new output file,
